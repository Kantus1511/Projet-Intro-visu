      ...
      for (int y = 0; y <= 1; y++)
          ...
          //  edge refers to our current node
        {
          ...
          } else if ((path.length == 4) && (x == path[path.length - 1]))
          {
            ...
            int[] inv = invert(p);
            PVector l1 = lines.get(path[0]);
            PVector l2 = lines.get(path[1]);
            PVector l3 = lines.get(path[2]);
            PVector l4 = lines.get(path[3]);
            ...
            if (isNew(p) && isNew(inv) && isConvex(c12, c23, c34, c41)
            && nonFlatQuad(c12, c23, c34, c41) && validArea(c12, c23, c34, c41
            , 250000, 10000)) {
              cycles.add(p);
              //println("Cycle added");
            }
